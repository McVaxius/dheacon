...
public sealed class AudioPlaybackService
{
    ...
    public void PlayAlert()
    {
        ...
        if (PlayWavFile(resolvedPath, "legacy alert"))
            return;
    ...
    public bool PlayWavFile(string resolvedPath, string label)
    {
        ...
        player.Play();
        log.Information($"[Dheacon] Played {label}: {resolvedPath}");
        return true;
    }

    public bool PlayWavFileSync(string resolvedPath, string label)
    {
        if (!File.Exists(resolvedPath))
            return false;
